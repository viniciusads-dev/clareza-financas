...
export async function handleApi(request:Request,env:Env):Promise<Response>{
 try {
  ...
  if(!owner)throw new HttpError(401,'Entre na sua conta para acessar suas finanças.');
  if(!env.DB)throw new HttpError(503,'Não foi possível acessar seus dados. Tente novamente.');
  const path=url.pathname.replace(/^\/api\/?/,'').split('/').filter(Boolean);
  if(request.method==='GET'&&path[0]==='state'){return json(await readState(env.DB,owner));}
  if(!['POST','PUT','DELETE'].includes(request.method))throw new HttpError(405,'Operação não permitida.');
  const origin=request.headers.get('origin');
  if(!origin||origin!==url.origin||request.headers.get('sec-fetch-site')==='cross-site')throw new HttpError(403,'Origem não autorizada.');
