...
const incomeCategories = ["Salário", "Freelance", "Outros"];
...
const shortDate = (date: string) =>
  new Date(`${date}T12:00:00`)
    ...
    .replace(".", "");
const categoryType = (category: string): "expense" | "income" =>
...
export default function FinanceApp({
  ...
  const [focus, setFocus] = useState(() => {
    try {
      return (
        ...
        localStorage.getItem("clareza-focus") === "true"
      );
    } catch {
      return false;
    }
  ...
  const [hidden, setHidden] = useState(() => {
    try {
      return (
        typeof window !== "undefined" &&
        localStorage.getItem("clareza-hidden") === "true"
      );
    } catch {
      return false;
    }
  ...
  // A leitura do estado é uma sincronização assíncrona necessária com a API.
  useEffect(() => {
  ...
  const state = useMemo(
    () => (demo ? demoState(month) : real),
    [demo, month, real],
  );
  const stats = useMemo(() => monthStats(state, month), [month, state]);
  const previousStats = useMemo(
    () => monthStats(state, addMonths(`${month}-01`, -1).slice(0, 7)),
    [month, state],
  );
  ...
  const previousCategoryData = Object.entries(
    previousStats.tx
      .filter((transaction) => transaction.type === "expense")
      .reduce<Record<string, number>>((result, transaction) => {
        result[transaction.category] =
  ...
  }, [categoryData, monthlyBudgets, previousCategoryData, state.recurrences]);
  ...
  function preference(which: "focus" | "hidden", value: boolean) {
    if (which === "focus") setFocus(value);
    else setHidden(value);
    try {
      localStorage.setItem(`clareza-${which}`, String(value));
    } catch {
  ...
  function openEditor(
    ...
    if (demo) {
      toast(
        "Comece cadastrando sua primeira conta para personalizar o espaço.",
        { action: { label: "Começar", onClick: start } },
      );
      return;
    }
    resetOperation();
    ...
    setEditor({ kind, id: record?.id, values });
  }
  function start() {
    setOnboarding({
      accountName: "Conta principal",
  ...
  function askDelete(kind: FormKind, id: string, name: string) {
    if (demo) {
      toast("Os dados desta prévia são fictícios.");
      return;
    }
  ...
  return (
    <SidebarProvider style={{ "--sidebar-width": "235px" } as CSSProperties}>
        ...
        <main className="main-content">
          ...
          {loadError ? (
            <div className="error-card" role="alert">
              <h2>Não conseguimos carregar suas finanças.</h2>
              <p>{loadError}</p>
            ...
            <div className="loading-grid" aria-label="Carregando finanças">
            ...
            <>
              {view === "overview" && (
                <>
                  ...
                  <div className="dashboard-grid">
                    ...
                    <aside className="right-column">
                      <section className="panel agenda">
                        ...
                        {agendaItems.length ? (
                          agendaItems.slice(0, 4).map(agendaRow)
                        ) : (
                      ...
                      <section className="panel secondary budget-overview">
                        <div className="section-head">
                  ...
                  <section className="panel goals-overview secondary">
                    <div className="section-head">
                      <div>
                    ...
                    {state.goals.length ? (
                      <div className="goals-grid">
                        {state.goals.slice(0, 2).map(goalCard)}
                      </div>
                    ) : (
                      <Empty
      ...
      <Dialog
        open={onboardingOpen}
        onOpenChange={(open) => {
          if (!saving) setOnboardingOpen(open);
        }}
      >
